 ...
 * @file:       DemoAdaptor.h
 *
 * Purpose:     Class declaration for DemoAdaptor.
 *
 *
 ...
 *
 */
class DemoAdaptor : public imaqkit::IAdaptor
{
  ...
  public:
    ...
    //  -  Handle to one of the imaqkit::IDeviceInfo objects you created in getAvailHW()
    ...
    DemoAdaptor(imaqkit::IEngine* engine, const imaqkit::IDeviceInfo* deviceInfo, const char* formatName);
    virtual ~DemoAdaptor();
     ...
     * the correct values. If a property is not correctly configured, the property
     * listener should automatically configure it.
     *
     ...
     *
     * @return bool: True is startCapture() has been called, false once stopCapture() is called.
     */
  ...
  private:
     ...
     * listeners.
     * initDevice() is invoked in the DemoAdaptor constructor.
     *
     * @return void:
     */
    void initDevice();
    ...
    /**
     * getFormatInfo: Return the image acquisition device's DemoDeviceFormat
     * object. The DemoDeviceFormat object contains image width, image height,
     * number of color bands, and colorspace information.
     *
     * @return DemoDeviceFormat: The device format information object.
     */
    DemoDeviceFormat* getFormatInfo() const;

    /**
     * sendThread: The ThreadProc responsible for acquiring frames from the
     * device and sending those frames back to the engine.
     *
     * @param param: Parameter value to be cast to DemoDeviceFormat.
     *
     * @return ThreadReturnType:
     */
    static ThreadReturnType CALLING_CONVENTION sendThread(void* param);

    static void sendFrame(DemoAdaptor* adaptor);
    ...
    imaqkit::IEnginePropContainer* _enginePropContainer;
